    ...
    return (
      <div className="fixed inset-0 z-[100] bg-slate-950 flex items-center justify-center p-6 text-center">
        <div className="max-w-md w-full bg-slate-900 p-8 rounded-2xl border border-purple-500/50 shadow-2xl">
          <h1 className="text-3xl font-black text-purple-500 mb-4 italic text-center">PAK VAPE STORE</h1>
    ...
    <div className="min-h-screen bg-[#0B0E14] text-white">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        <div className="mb-10 text-center">
          <h2 className="text-4xl font-black mb-2 uppercase tracking-tighter">Premium Vapes & Drops</h2>
          <p className="text-slate-400 text-sm italic flex items-center justify-center gap-1">
            <MapPin className="w-3 h-3 text-purple-500" /> Aryaan Vape Shop - Shop 7, Ithad Plaza, Gujar Khan
          </p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {filtered.map(p => (
            <div key={p.id} className="bg-white/5 rounded-xl border border-white/10 overflow-hidden hover:border-purple-500/50 transition">
              <img src={p.images[0]} className="w-full aspect-square object-cover" alt={p.name} />
              <div className="p-4">
                <h3 className="font-bold text-sm truncate">{p.name}</h3>
                <p className="text-purple-500 font-black text-sm mt-1">Rs. {p.price}</p>
                <button 
                  onClick={() => window.location.href=`/product/${p.id}`}
                  className="w-full mt-3 bg-white text-black py-2 rounded-lg text-xs font-bold hover:bg-purple-500 hover:text-white transition"
                >
                  VIEW DETAILS
                </button>
              </div>
